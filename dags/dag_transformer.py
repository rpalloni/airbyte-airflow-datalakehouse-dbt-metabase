...
run_task = DockerOperator(
    task_id="run_dbt", command="dbt run -s +rpt_user_stats", **DOCKER_OPERATOR_DEFAULTS
)

test_task = DockerOperator(
    task_id="test_dbt", command="dbt test", **DOCKER_OPERATOR_DEFAULTS
)

end = EmptyOperator(task_id="workflow_end", dag=dag)

start >> run_task >> test_task >> end
